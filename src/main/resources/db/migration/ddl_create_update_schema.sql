...
create table commands (merchant_sequence integer not null, table_index integer, id uuid not null, merchant_id uuid, name varchar(255), status varchar(255) check (status in ('OPENED','CLOSED')), primary key (id));
...
create table merchant_logistic_settings (delivery_fee_per_km numeric(10,4) not null, free_delivery_radius_km numeric(10,2), max_delivery_radius_km numeric(10,2) not null, min_delivery_fee numeric(10,2) not null, id uuid not null, primary key (id));
...
create table order_items (options_price numeric(10,2), quantity integer, total_price numeric(10,2), context_modifier_id uuid not null, id uuid not null, item_id uuid, order_id uuid, primary key (id));
